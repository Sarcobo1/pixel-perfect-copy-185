...
<style>
  ...
  .card{
    background:linear-gradient(145deg,var(--card-bg) 0%,transparent 100%);
    border:1px solid var(--border);
    box-shadow:0 0 0 1px rgba(255,255,255,0.06) inset,0 24px 64px rgba(0,0,0,0.12),0 0 48px var(--glow);
    border-radius:32px;padding:52px 36px;min-width:300px;flex:1;
  ...
  .glass-panel {
    ...
    border: 1px solid var(--border);
    box-shadow: 0 32px 64px rgba(0,0,0,0.12), inset 0 1px 0 rgba(255,255,255,0.1);
    position: relative;
    overflow: hidden;
  }
  .glass-panel::before {
...
window.__sota_animations=${JSON.stringify(
  ...
  private renderScene(scene: MotionScene, index: number): string {
    ...
    switch (scene.layout) {
      ...
      case "title_card":
        inner = `<div class="content" style="width:100%;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;background:radial-gradient(circle at center, var(--card-bg) 0%, var(--bg) 100%);">
  ${scene.headline ? `<div class="headline" id="${id}-headline" style="font-size:140px;line-height:1.1;letter-spacing:-0.02em;text-align:center;">${this.wrapWords(scene.headline)}</div>` : ""}
  ${scene.subheadline ? `<div class="subheadline" id="${id}-sub" style="margin-top:32px;font-size:32px;letter-spacing:0.1em;color:var(--accent);">${scene.subheadline}</div>` : ""}
</div>`;
        break;
...
<script id="${id}-kinetic-data" type="application/json">${JSON.stringify({words:kWords,colors:kColors,wordDurationMs:scene.kineticWordDurationMs||900,exitDurationMs:scene.kineticExitDurationMs||500,staggerMs:80})}</script>`;
        break;
      }
...
<script id="${id}-stack-data" type="application/json">${JSON.stringify({ words: stackWords, wordIntervalMs: wordInterval })}</script>`;
      ...
      case "word_slide_mask": {
        // Premium masked word reveal — each word slides up from behind a hidden horizon
        const slideText = (scene as any).text || scene.headline?.replace(/<[^>]+>/g, "") || "Premium Motion";
        const slideWords = slideText.split(/\s+/).filter(Boolean);
        const maskWords = slideWords.map((w: string, wi: number) =>
          `<span style="display:inline-block;overflow:hidden;vertical-align:bottom;margin-right:0.25em;">
            <span class="slide-word" id="${id}-sw-${wi}" style="display:inline-block;transform:translateY(110%);opacity:0;">${w}</span>
          </span>`
        ).join("");
        inner = `<div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;">
  <div style="font-size:clamp(64px,7vw,120px);font-weight:900;letter-spacing:-0.04em;line-height:1.1;text-align:center;color:var(--text);">${maskWords}</div>
  ${scene.subheadline ? `<div id="${id}-sub" style="margin-top:32px;font-size:clamp(20px,2vw,30px);color:var(--muted);letter-spacing:0.05em;opacity:0;">${scene.subheadline}</div>` : ""}
</div>
<script id="${id}-slide-data" type="application/json">${JSON.stringify({ count: slideWords.length, intervalMs: (scene as any).word_interval_ms || 180 })}</script>`;
        break;
      }

      case "word_blur_focus": {
        // Each word crystallizes from blur into sharp focus
        const blurText = (scene as any).text || scene.headline?.replace(/<[^>]+>/g, "") || "Focus is a superpower";
        const blurWords = blurText.split(/\s+/).filter(Boolean);
        const blurEls = blurWords.map((w: string, wi: number) =>
          `<span class="blur-word" id="${id}-bw-${wi}" style="display:inline-block;margin-right:0.3em;opacity:0;filter:blur(${scene.enter_blur_px ?? 20}px);color:var(--text);">${w}</span>`
        ).join("");
        inner = `<div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;">
  <div style="font-size:clamp(60px,6.5vw,110px);font-weight:900;letter-spacing:-0.04em;line-height:1.15;text-align:center;">${blurEls}</div>
  ${scene.subheadline ? `<div id="${id}-sub" style="margin-top:28px;font-size:clamp(20px,1.8vw,28px);color:var(--muted);opacity:0;">${scene.subheadline}</div>` : ""}
</div>
<script id="${id}-blur-data" type="application/json">${JSON.stringify({ count: blurWords.length, intervalMs: (scene as any).word_interval_ms || 300, blurPx: scene.enter_blur_px ?? 20 })}</script>`;
        break;
      }
...
<script id="${id}-morph-data" type="application/json">${JSON.stringify({pairs,intervalMs:scene.morphInterval||2000})}</script>`;
  ...
  private buildTimeline(): void {
    ...
    scenes.forEach((scene, i) => {
      ...
      // word_slide_mask — masked upward word reveal
      if (scene.layout === "word_slide_mask") {
        this.tl.add(() => {
          const dataEl = document.getElementById(`${id}-slide-data`);
          if (!dataEl || dataEl.dataset.init) return;
          dataEl.dataset.init = "true";
          const { count, intervalMs } = JSON.parse(dataEl.textContent || "{}");
          const gsap = (window as any).gsap;
          const surgical = "cubic-bezier(0.16, 1, 0.3, 1)";
          for (let wi = 0; wi < count; wi++) {
            const el = document.getElementById(`${id}-sw-${wi}`);
            if (!el) continue;
            gsap.to(el, {
              y: "0%",
              opacity: 1,
              duration: 0.65,
              delay: wi * (intervalMs / 1000),
              ease: surgical,
            });
          }
          const sub = document.getElementById(`${id}-sub`);
          if (sub) {
            gsap.to(sub, { opacity: 1, y: 0, duration: 0.5, delay: count * (intervalMs / 1000) + 0.1, ease: surgical });
          }
        }, t + 0.15);
      }

      // word_blur_focus — each word crystallizes from blur
      if (scene.layout === "word_blur_focus") {
        this.tl.add(() => {
          const dataEl = document.getElementById(`${id}-blur-data`);
          if (!dataEl || dataEl.dataset.init) return;
          dataEl.dataset.init = "true";
          const { count, intervalMs, blurPx } = JSON.parse(dataEl.textContent || "{}");
          const gsap = (window as any).gsap;
          const fluid = "cubic-bezier(0.22, 1, 0.36, 1)";
          for (let wi = 0; wi < count; wi++) {
            const el = document.getElementById(`${id}-bw-${wi}`);
            if (!el) continue;
            gsap.to(el, {
              opacity: 1,
              filter: "blur(0px)",
          ...
          const sub = document.getElementById(`${id}-sub`);
          if (sub) {
            gsap.to(sub, { opacity: 1, duration: 0.5, delay: count * (intervalMs / 1000) + 0.1, ease: fluid });
          }
        }, t + 0.1);
      }
  ...
  private applyTransitionLegacy(from: string, to: string, type: string, t: number): void {
    ...
    // Unknown transition — graceful fade fallback
    if (!transitions[type]) {
      if (direction === "out") {
  ...
  private addCameraMove(selector: string, move: string, t: number, dur: number): void {
    ...
    const moves: Record<string, () => void> = {
      zoom_in:        () => this.tl.to(el, { scale: 1.08, duration: dur, ease: "none" }, t),
      zoom_out:       () => this.tl.to(el, { scale: 0.94, duration: dur, ease: "none" }, t),
      push_through:   () => this.tl.to(el, { scale: 1.2, filter: "blur(20px)", duration: 0.3, ease: "power3.in" }, t),
      pan_left:       () => this.tl.to(el, { x: -40, duration: dur, ease: "none" }, t),
      pan_right:      () => this.tl.to(el, { x: 40, duration: dur, ease: "none" }, t),
      shake:          () => this.tl.to(el, { x: 6, duration: 0.05, yoyo: true, repeat: 6 }, t),
      // Quiet Luxury micro-movements
      zoom_in_micro:  () => this.tl.fromTo(el, { scale: 1.0 }, { scale: 1.03, duration: dur, ease: "linear" }, t),
      pan_right_slow: () => this.tl.fromTo(el, { x: -10 }, { x: 10, duration: dur, ease: "linear" }, t),
      tilt_up_subtle: () => this.tl.fromTo(el, { y: 10 }, { y: -10, duration: dur, ease: "linear" }, t),
    };
    moves[move]?.();
  }
  ...
  private async loadGSAP(): Promise<void> {
    if ((window as any).gsap) {
      // Register CustomEase curves if not already done
      this.registerPremiumEases();
      return;
    }
    return new Promise((res) => {
      ...
      s.onload = () => {
        // Load CustomEase plugin after core GSAP
        const ce = document.createElement("script");
        ce.src = "https://cdn.jsdelivr.net/npm/gsap@3.12.5/dist/CustomEase.min.js";
        ce.onload = () => { this.registerPremiumEases(); res(); };
        ce.onerror = () => res(); // graceful fallback if CDN fails
  ...
  private registerPremiumEases(): void {
    const g = (window as any).gsap;
    const CE = (window as any).CustomEase;
    if (!g || !CE || (window as any).__sota_eases_registered) return;
    try {
      CE.create("linear-surgical", "M0,0 C0.16,1 0.3,1 1,1");
      CE.create("apple-fluid",     "M0,0 C0.25,1 0.5,1 1,1");
      (window as any).__sota_eases_registered = true;
    } catch(e) { /* CustomEase unavailable, use string fallbacks */ }
  }
}
